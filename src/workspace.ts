...
interface HistoryEntry { at: string; mode: string; suite: string; status: "passed" | "failed"; durationMs: number; tests: Array<{ name: string; status: string; durationMs: number; error?: string }> }
...
async function appendHistory(path: string, entry: HistoryEntry): Promise<void> { try { await mkdir(dirname(path), { recursive: true }); const existing = await readHistory(path); existing.push(entry); const trimmed = existing.slice(-HISTORY_LIMIT); await writeFile(path, trimmed.map((item) => JSON.stringify(item)).join("\n") + "\n", "utf8"); } catch { /* history is best-effort */ } }
async function readHistory(path: string): Promise<HistoryEntry[]> { try { const text = await readFile(path, "utf8"); if (text.length > 20 * 1024 * 1024) return []; return text.split("\n").filter(Boolean).flatMap((line) => { try { const value = JSON.parse(line); return value && typeof value === "object" ? [value as HistoryEntry] : []; } catch { return []; } }); } catch { return []; } }
