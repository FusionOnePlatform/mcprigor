...
const DESCRIPTIONS = {
  ...
  "authentication": "Test authenticated MCP servers with MCP Rigor: static bearer tokens, API-key and custom headers, interactive browser-redirect OAuth carried across tests, fetched short-lived tokens, the ${env.NAME} placeholder, project environments, and CI secrets — with automatic redaction.",
...
const FAQ = [
  ...
  ["How do I test an MCP server that requires authentication?", "Point the suite at the Streamable HTTP endpoint and pass credentials as headers, using ${env.NAME} placeholders so no secret lives in the test file — for example Authorization: \"Bearer ${env.QA_TOKEN}\". API keys and custom headers work the same way, short-lived OAuth tokens can be fetched at run time with Token from, and for servers that need a real user login, OAuth: oauth runs an interactive browser authorization-code flow once and carries the refreshed session into every test. Every resolved token is redacted from reports. See the Authentication guide."],
...
const NAV = [
  ...
  { section: "Engineering", pages: [
    ...
    ["AUTHENTICATION", "Authentication & secrets"],
